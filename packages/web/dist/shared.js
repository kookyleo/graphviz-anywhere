...
export async function loadDefaultVizWasmModule() {
    // Import the compiled Wasm module from the published package first.
    // This module is generated by emscripten with MODULARIZE=1
    let vizModuleFactory;
    try {
        // Try to import from @kookyleo/graphviz-anywhere-web wasm export first (npm publish case)
        vizModuleFactory = await import('@kookyleo/graphviz-anywhere-web/wasm');
    }
    catch {
        // Fallback to output/wasm for development/build scenarios
        try {
            ...
            throw new Error('Wasm module not found. Make sure to build the Wasm module first: npm run build');
        }
    }
