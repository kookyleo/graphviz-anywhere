//! Core FFI bindings to the graphviz-anywhere C ABI.
//!
...
extern "C" {
    ...
    /// Get the list of available layout engines as JSON.
    pub fn gv_get_engines() -> *const c_char;

    /// Get the list of available output formats as JSON.
    pub fn gv_get_formats() -> *const c_char;

    /// Get the render format details for a specific format as JSON.
    pub fn gv_render_formats(format: *const c_char) -> *const c_char;

    /// Get the Graphviz library version string.
    pub fn gv_get_version() -> *const c_char;
}
